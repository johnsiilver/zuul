...
import (
	...
	"github.com/johnsiilver/zuul/internal/dragonboat/internal/id"
	"github.com/johnsiilver/zuul/internal/dragonboat/internal/settings"
	"github.com/johnsiilver/zuul/internal/dragonboat/internal/vfs"
	"github.com/johnsiilver/zuul/internal/dragonboat/logger"
	"github.com/johnsiilver/zuul/internal/dragonboat/raftio"
	pb "github.com/johnsiilver/zuul/internal/dragonboat/raftpb"
)
...
func (tm *defaultTransport) Validate(addr string) bool {
	return isValidDualStackAddress(addr)
}
...
func (c *NodeHostConfig) GetTargetValidator() TargetValidator {
	...
	return isValidDualStackAddress
}
...
func (g *GossipConfig) Validate() error {
	if len(g.BindAddress) > 0 && !isValidDualStackAddress(g.BindAddress) {
...
func isValidAdvertiseAddress(addr string) bool {
	// The advertise address must be a literal IP:port (memberlist cannot use a
	// hostname), but may be IPv4 OR IPv6. (Zuul fork patch: was IPv4-only via
	// stringutil.IPV4Regex; now net/netip.)
	ap, err := netip.ParseAddrPort(addr)
	return err == nil && ap.Port() != 0
}

// isValidDualStackAddress reports whether addr is a valid "host:port" where host
// may be a hostname, an IPv4 literal, or an IPv6 literal. This replaces goutils'
// IPv4/hostname-only stringutil.IsValidAddress so dragonboat (and Zuul) accept IPv6.
// (Zuul fork patch — see PRP/security-audit-dragonboat.md.)
func isValidDualStackAddress(addr string) bool {
	// IPv4/IPv6 literal "ip:port".
	if ap, err := netip.ParseAddrPort(addr); err == nil {
		return ap.Port() != 0
	}
	// Otherwise "host:port" with a hostname.
	...
	return stringutil.HostnameRegex.MatchString(host)
}
