...
func (v *Verifier) Verify(ctx context.Context, raw string) (string, error) {
	...
	if err != nil {
		return "", errors.E(ctx, errors.CatUnauthenticated, errors.TypeInvalidToken, fmt.Errorf("oidcauth: invalid token: %w", err))
	}
	claims := map[string]any{}
	if err := token.Claims(&claims); err != nil {
		return "", errors.E(ctx, errors.CatUnauthenticated, errors.TypeInvalidToken, fmt.Errorf("oidcauth: decode claims: %w", err))
	}
	identity, ok := claims[v.claim].(string)
	if !ok || identity == "" {
		return "", errors.E(ctx, errors.CatUnauthenticated, errors.TypeInvalidToken, fmt.Errorf("oidcauth: token has no usable %q claim", v.claim))
	}

	if v.cache.Len() < maxTokenCache {
		v.cache.Set(raw, cacheEntry{identity: identity, exp: cacheUntil(now, token.Expiry)})
	}
	return identity, nil
}
