...
// ownership (see internal/auth/authz).
package keypath
...
// HasPrefix reports whether key falls under prefix on a path boundary. An empty
// prefix is a wildcard (matches every key). Otherwise the match is on a path
// boundary, not a raw string prefix: prefix "/a/configs" covers "/a/configs" and
// "/a/configs/..." but NOT the sibling keys "/a/configs-secret" or "/a/configsX". A
// trailing "/" on the prefix is ignored, so "/a/configs" and "/a/configs/" behave
// identically. It does not validate either argument.
func HasPrefix(prefix, key string) bool {
	if prefix == "" {
		return true
	}
