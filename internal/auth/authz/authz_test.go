...
func TestPrefixAuthorize(t *testing.T) {
	a := Prefix(map[string][]Rule{
		...
		"alice":      {{Prefix: "/alice/configs", Write: true}},
	})
	...
	tests := []struct {
		...
		// Regression: a prefix grant must match on a path boundary, not a raw string
		// prefix, so it cannot leak to sibling keys that merely share a string prefix.
		{name: "Success: exact prefix key is covered", identity: "alice", key: "/alice/configs", op: Write},
		{name: "Success: subtree under prefix is covered", identity: "alice", key: "/alice/configs/db", op: Write},
		{name: "Error: sibling key with shared string prefix denied", identity: "alice", key: "/alice/configs-secret/db", op: Read, wantErr: true},
		{name: "Error: sibling key without separator denied", identity: "alice", key: "/alice/configsX", op: Read, wantErr: true},
	}
