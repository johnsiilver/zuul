...
// defaultSnapshotEntries is the Raft log length that triggers a snapshot + compaction when
// Config.SnapshotEntries is 0. It is deliberately well below dragonboat's usual disk-backed
// default: Zuul keeps the Raft log in RAM (NewMem VFS), so every retained entry is process
// memory. Each hosted shard holds up to this many entries (with election/lock values up to
// MaxRecvBytes) before truncating, so a high value lets the in-RAM LogDB grow large under
// sustained writes. A soak measuring live HeapAlloc settled the value: 10000 climbed past
// 800 MiB and kept rising; 2000 plateaued ~308 MiB; 1000 plateaued ~230 MiB (fastest, dead
// flat) with no throughput cost — and below ~1000 the savings hit a fixed floor (the
// ~84 MiB of per-connection Raft transport buffers), so 1000 is the sweet spot.
const defaultSnapshotEntries = 1_000
...
type Config struct {
	...
	// compaction (in the in-memory FS); 0 means defaultSnapshotEntries (1000).
...
type Host struct {
	...
	shards      immutable.Slice[uint64] // lock shards (expiry sweeps these)
	hosted      immutable.Slice[uint64] // every hosted shard (lock shards + meta shard)
	metaShardID uint64
	replicaID   uint64
	raftAddr    string
	grpcAddr    string
...
func New(ctx context.Context, cfg Config) (*Host, error) {
	...
	if snapEntries == 0 {
		snapEntries = defaultSnapshotEntries
	}
	...
	h := &Host{
		...
		shards:      immutable.NewSlice(append([]uint64(nil), cfg.Shards...)),
		hosted:      immutable.NewSlice(hosted),
...
func (h *Host) Shards() immutable.Slice[uint64] {
	return h.shards
}

// Hosted returns every shard id this node hosts (lock shards plus the meta shard).
func (h *Host) Hosted() immutable.Slice[uint64] {
	return h.hosted
}
...
func (h *Host) waitReady(ctx context.Context) error {
	wait, cancel := context.EnsureDeadline(ctx, proposeTimeout)
	if cancel != nil {
		defer cancel()
	}
...
func (h *Host) allShardsHaveLeader() bool {
	for _, shardID := range h.hosted.All() {
...
func (h *Host) Propose(ctx context.Context, shardID uint64, cmd []byte) ([]byte, error) {
	...
	pctx, cancel := context.EnsureDeadline(ctx, proposeTimeout)
	if cancel != nil {
		defer cancel()
	}
...
func (h *Host) Read(ctx context.Context, shardID uint64, query any) (any, error) {
	rctx, cancel := context.EnsureDeadline(ctx, proposeTimeout)
	if cancel != nil {
		defer cancel()
	}
	...
	task := func(ctx context.Context) error {
		...
		for {
			...
			case <-ctx.Done():
				return nil
			case <-tick.C:
				for _, shardID := range h.shards.All() {
					if !h.IsLeader(shardID) {
						continue
					}
