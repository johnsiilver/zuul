...
// ListLocksQuery enumerates the held lock/election keys, optionally restricted to
// those covered by Prefix (a path-boundary match; an empty Prefix returns all). It
// backs the read-only browsing UI.
type ListLocksQuery struct {
	// Prefix narrows the result to keys under it; "" returns every held key.
	Prefix string
}

// LockSummary is one row of a ListLocksQuery result.
type LockSummary struct {
	// Name is the lock/election key.
	Name string
	// Held reports whether the key currently has a holder/leader (always true in a
	// ListLocksQuery result, which omits unheld keys).
	Held bool
	// Holder is the holder/leader's client id.
	Holder string
	// Token is the holder's fencing token.
	Token uint64
	// HasValue reports whether a published value is present (a best-effort signal that
	// the key is an election rather than a plain lock).
	HasValue bool
	// QueueDepth is the number of waiters behind the holder.
	QueueDepth uint32
}

// LockSummaries is the answer to a ListLocksQuery, sorted by Name.
type LockSummaries struct {
	// Locks are the matching held locks/elections, sorted by Name.
	Locks []LockSummary
	// Revision is the FSM logical clock at which this was read.
	Revision uint64
}

// WaitersQuery asks for the holder plus the FIFO wait-queue contenders behind one key.
type WaitersQuery struct {
	// Name is the lock/election key.
	Name string
}

// WaiterInfo is one entry of a key's FIFO wait queue.
type WaiterInfo struct {
	// ClientID is the waiting client.
	ClientID string
	// Seq is the waiter's monotonic enqueue order.
	Seq uint64
	// Position is the 1-based position in the queue.
	Position uint32
}

// Waiters is the answer to a WaitersQuery.
type Waiters struct {
	// Held reports whether the key currently has a holder/leader.
	Held bool
	// Holder is the holder/leader's client id; "" when not held.
	...
	// Value is the election leader's published value; nil for plain locks.
	Value []byte
	// Entries are the contenders in FIFO (queue) order.
	Entries []WaiterInfo
	// Revision is the FSM logical clock at which this was read.
	Revision uint64
}

// Query answers a read. q is a StatusQuery, LeaseQuery, DueLeasesQuery, ListLocksQuery,
// or WaitersQuery; the result is the matching answer type. An unknown query type returns
// an error.
func (f *FSM) Query(q any) (any, error) {
	...
	case ListLocksQuery:
		return f.listLocks(query.Prefix), nil
	case WaitersQuery:
		return f.waiters(query.Name), nil
	default:
		return nil, fmt.Errorf("fsm.Query: unknown query type %T", q)
	}
}

// listLocks returns the held locks/elections whose key falls under prefix, sorted by
// key. Unheld entries (transient, holder == "") are skipped.
func (f *FSM) listLocks(prefix string) LockSummaries {
	out := LockSummaries{Revision: f.revision}
	for _, name := range sortedKeys(f.locks) {
		lk := f.locks[name]
		if lk.holder == "" || !keypath.HasPrefix(prefix, name) {
			continue
		}
		out.Locks = append(out.Locks, LockSummary{
			Name:       name,
			Held:       true,
			Holder:     lk.holder,
			Token:      lk.token,
			HasValue:   lk.value != nil,
...
// waiters reports the holder and the FIFO contenders behind name.
func (f *FSM) waiters(name string) Waiters {
	lk := f.locks[name]
	if lk == nil || lk.holder == "" {
		return Waiters{Revision: f.revision}
	}
	out := Waiters{
		Held:     true,
		Holder:   lk.holder,
	...
	for i, w := range lk.queue {
		out.Entries = append(out.Entries, WaiterInfo{ClientID: w.clientID, Seq: w.seq, Position: uint32(i + 1)})
	}
	return out
}
